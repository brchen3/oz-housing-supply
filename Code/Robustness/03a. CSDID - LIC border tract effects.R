...
USPS_data <- USPS_data %>%
  filter(`Designation_category_detailed` %in% c("LIC not selected, border tract","LIC not selected, not a border tract")) %>%
...
USPS_data <- USPS_data %>%
  ...
  mutate(
    G = if_else(`Designation_category_detailed` == "LIC not selected, border tract", period_value, 0),
...
setwd(path_output)
write.xlsx(reshaped_table, file = "CSDID LIC Border Effect Estimate.xlsx", overwrite = TRUE)
