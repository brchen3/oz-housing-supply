...
USPS_data <- USPS_data %>%
  filter(`Designation_category_detailed` %in% c("Contiguous not selected, border tract",
                                                "Contiguous not selected, not a border tract",
                                                "Ineligible, border tract",
...
USPS_data <- USPS_data %>%
  ...
  mutate(
    G = if_else(`Designation_category_detailed` %in% c("Contiguous not selected, border tract",
                                                       "Ineligible, border tract"),
                period_value, 
                0),
...
setwd(path_output)
write.xlsx(reshaped_table, file = "CSDID Non-LIC Border Effect Estimate.xlsx", overwrite = TRUE)
