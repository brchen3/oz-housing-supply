...
path_data_lodes <- file.path(path_project, "data/LODES")
...
# tract - crosswalk
setwd(path_data)
# tab20_tract20_tract10_natl <- read_delim("tab20_tract20_tract10_natl.txt", 
#                                          delim = "|", escape_double = FALSE, trim_ws = TRUE)
# CENSUS_TRACT_CROSSWALK_2010_to_2020_2023 <- read_excel("CENSUS_TRACT_CROSSWALK_2010_to_2020_2023.xlsx")
CENSUS_TRACT_CROSSWALK <- read_excel("CENSUS_TRACT_CROSSWALK_2010_to_2020_2010.xlsx")

NCWM_Economic_Development2 <- read_excel("NCWM_Economic_Development2.xlsx")
...
NCWM_Economic_Development2 <- NCWM_Economic_Development2 %>%
  mutate(geoid = as.numeric(GEOID))

# For the ACS data, crosswalk using the residential ratio from the HUD data on addresses since the variables are at the person level
load("ACS_2012_2023_2020tracts.RData")
ACS_2012_2023 <- ACS_2012_2023 %>% rename(YEAR = year) %>% mutate(YEAR = as.character(YEAR))
openxlsx::write.xlsx(ACS_2012_2023, file = "ACS_2012_2023_2020tracts.xlsx")
...
#########
# post_clean_states <- USPS_data %>%
#   group_by(state_fips, YEAR_MONTH) %>%
#   summarise(count_post = n())
# 
# cleaned_state <- pre_clean_states %>% 
#   left_join(post_clean_states) %>%
#   mutate(
#     count_post = if_else(is.na(count_post),0,count_post),
#     change = count_pre - count_post) %>%
#   arrange(-change)
